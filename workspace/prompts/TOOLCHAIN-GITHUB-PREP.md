...
- Search for hardcoded secrets: `grep -rn "sk_live\|pk_live\|api_key\|apiKey\|secret" --include="*.ts" --include="*.tsx" --include="*.py" .`
...
- Project title: **ToolChain**
- Live demo link: https://toolchain.vercel.app
...
- Multi-agent orchestration (if LangGraph/LangChain present)
- RAG / Vector embeddings (if vector DB present)
- Vercel AI SDK / Streaming (if AI streaming present)
...
git init
git add .
git commit -m "Initial commit: ToolChain shell"
git branch -m main
gh repo create toolchain-shell --public --source=. --push
```
