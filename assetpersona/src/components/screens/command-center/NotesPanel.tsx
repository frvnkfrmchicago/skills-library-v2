...
import { Plus, Check, Clock, Circle, Sparkles, Send } from 'lucide-react';
...
export function NotesPanel({ notes, onAdd, onUpdate, onDelete, onExtractThemes, accent, accentSoft, txtMuted, txtHeadline, hairline }: Props) {
        ...
        {expanded && (
          <div style={{ marginTop: spacing.md, borderTop: `1px solid ${hairline}`, paddingTop: spacing.md }}>
            ...
            <div style={{ display: 'flex', gap: spacing.sm }}>
              ...
              />
              <Button size="sm" onClick={() => addUpdate(note)}><Send size={14} />Send</Button>
            </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg, padding: spacing.lg, height: '100%', overflowY: 'auto' }}>
      ...
      {isAdding ? (
        ...
        <Button onClick={() => setIsAdding(true)} size="md" style={{ alignSelf: 'flex-start' }}><Plus size={16} />New Note</Button>
      )}
