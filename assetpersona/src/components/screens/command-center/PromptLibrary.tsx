    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg, padding: spacing.lg, height: '100%', overflow: 'hidden' }}>
        ...
        {(['all', 'system', 'user', 'chain', 'agent', 'creative', 'general'] as const).map((c) => (
          <button key={c} onClick={() => setCatFilter(c)}
            style={{
              ...
              background: catFilter === c ? `${colors.brandGreen}22` : 'transparent',
              color: catFilter === c ? colors.brandGreen : colors.text.muted,
            }}>
            {c}
          </button>
        ))}
          ...
          <Card key={prompt.id} padding="md">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <div style={{ display: 'flex', gap: spacing.xs, marginLeft: spacing.sm, flexShrink: 0 }}>
                ...
                <button onClick={() => setEditing({ ...prompt })} style={{ fontSize: typeScale.sm, fontWeight: fontWeight.semibold, color: colors.brandGreen, background: 'none', border: 'none', cursor: 'pointer' }}>Edit</button>
      ...
      {editing && (
        <div style={{
          ...
          background: 'rgba(0,0,0,0.85)', border: `2px solid ${colors.brandGreen}`, borderRadius: borderRadius.xl,
