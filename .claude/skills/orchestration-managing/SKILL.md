...
| 2 | **Research the pain points (GROUND, don't scrape)** | For the *specific* feature, gather current best practices and the real, citable pain points — not generic advice. Grounding ≠ collecting links: distill the *concept/principle/trait* from every source (2026 URL, library skill, librarian) and bind it to a concrete decision here. | A tight synthesis + real 2026 web URLs per problem area (never fabricated links), each reduced to an applied rule, not just a citation. |
| 3 | **Synthesize with the library** | Pull Gates 1 + 2 together using Skills Library V2. Frame every lane around the GAP found in Gate 1, never a greenfield rebuild. | Each planned lane cites ≥1 SKILL + ≥1 LIBRARIAN + ≥1 2026 URL (multiple encouraged; all three TYPES required) AND states the distilled rule it applies from each — citations without an applied rule are name-dropping. |
| 4 | **Decompose for parallel execution** | Split into as many file-exclusive lanes as the work needs — **no cap on agent count.** Batch by dependency. Maximize parallelism *within* the exclusive-ownership rule (file-exclusivity is the only governor). | A file-ownership map (no two agents on one file) + batch grouping + parallelism reasoning. |
