...
function ScreenNodeCard({ id, data }: NodeProps<ScreenNode>) {
  ...
  const tint = data.groupColor || accent;
...
function FlowChartInner({
  ...
  /* Commit a follow step on the map: record the trail, set focus, recenter. */
  const followNode = useCallback(
    (node: FlowNode) => {
      setHoverId(null);
      setMapFocusId((prev) => {
        if (prev && prev !== node.id) {
          // push the node we're leaving so ← can retrace it
          if (walkHistory.current[walkHistory.current.length - 1] !== prev) {
            walkHistory.current.push(prev);
          }
        }
        return node.id;
      });
