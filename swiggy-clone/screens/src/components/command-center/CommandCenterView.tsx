...
import { Command as CommandIcon, Play, ExternalLink } from 'lucide-react';
import type { TaskStatus, FeatureStatus, ComplianceItem, LegalDocument, ReportEntry, PromptEntry, PromptCategory } from '@/lib/command-center/types';
import {
  listNotes, saveNote, deleteNote, toggleNoteComplete,
  ...
  listPrompts, savePrompt, deletePrompt, createPrompt,
        ...
        <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
          {section === 'notes' && (
            <NotesPanel notes={notes}
              ...
              onToggleComplete={(id) => { toggleNoteComplete(id); refresh(); }} onExtractThemes={extractThemes}
              accent={ACCENT} accentSoft={ACCENT_SOFT} txtMuted={TXT_MUTED} txtHeadline={TXT_HEADLINE} hairline={HAIRLINE} />
          )}
          {section === 'tasks' && (
            <TasksPanel tasks={tasks}
              onAdd={(t) => { saveTask(t); refresh(); }} onUpdate={(t) => { saveTask(t); refresh(); }} onDelete={(id) => { deleteTask(id); refresh(); }}
              onSetStatus={(id, s) => { setTaskStatus(id, s); refresh(); }}
              accent={ACCENT} accentSoft={ACCENT_SOFT} txtMuted={TXT_MUTED} txtHeadline={TXT_HEADLINE} hairline={HAIRLINE} />
          )}
          {section === 'features' && (
            <FeatureTracker features={features} clones={[]}
              onAdd={(f) => { saveFeature(f); refresh(); }} onUpdate={(f) => { saveFeature(f); refresh(); }} onDelete={(id) => { deleteFeature(id); refresh(); }}
          ...
          {section === 'orchestration' && (
            <OrchestrationPanel waves={waves} accent={ACCENT} accentSoft={ACCENT_SOFT} txtMuted={TXT_MUTED} txtHeadline={TXT_HEADLINE} hairline={HAIRLINE} />
          )}
          {section === 'reports' && (
            <ReportsPanel reports={reports} onGenerate={generateReport} accent={ACCENT} accentSoft={ACCENT_SOFT} txtMuted={TXT_MUTED} txtHeadline={TXT_HEADLINE} hairline={HAIRLINE} />
          )}
          {section === 'prompts' && (
          ...
          {section === 'apps' && (
            <AppsPlatformsPanel accent={ACCENT} txtMuted={TXT_MUTED} txtHeadline={TXT_HEADLINE} hairline={HAIRLINE} onOpenApp={onOpenApp} />
          )}
          {section === 'assets' && (
