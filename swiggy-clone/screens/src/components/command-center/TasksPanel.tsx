...
import { Plus, Trash2, Check, Clock, Circle, AlertTriangle, Send, ChevronDown } from 'lucide-react';
...
export function TasksPanel({ tasks, onAdd, onUpdate, onDelete, onSetStatus, accent, accentSoft, txtMuted, txtHeadline, hairline }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg, padding: spacing.lg, height: '100%', overflowY: 'auto' }}>
      ...
      {isAdding ? (
        ...
        <Button onClick={() => setIsAdding(true)} size="md" icon={Plus} style={{ alignSelf: 'flex-start' }}>New Task</Button>
      )}
