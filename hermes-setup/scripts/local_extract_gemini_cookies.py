...
def main():
    ...
    temp_db = '/tmp/chrome_gemini_cookies_temp'
    shutil.copyfile(cookie_db, temp_db)

    conn = sqlite3.connect(temp_db)
    cursor = conn.cursor()
    
    # Query for Google and Gemini sessions
    query = """
    ...
    output_file = "/Users/franklawrencejr./Downloads/skills-library-v2 2/gemini_cookies_profile1.json"
    with open(output_file, "w") as f:
        json.dump({"cookies": formatted_cookies}, f, indent=2)
        
    print(f"\n[SUCCESS] Decrypted and saved Google/Gemini cookies to: {output_file}")
