...
- **Build interactive prototypes with real markup** — buttons, inputs, cards, nav bars as actual HTML/CSS. Never produce a gallery of screenshots, image grid, or page of `<img>` tags as the deliverable.
- **Animations must be unique per project.** Do not reuse CSS transitions, @keyframes, or JS animation logic across different clones. Each site/app has its own motion language — learn it from the source and build accordingly.
- **No placeholder shapes.** Decorative geometric shapes, random corners, generic filler visuals get rejected. Every element must correspond to something in the original.
...
DO NOT consider a clone complete without **click-through verification**:

1. **Open the file** — it renders in a browser
2. **Click through every screen** — navigate to each screen/section, verify the `.active` class toggles, check that interactive elements respond (buttons, toggles, inputs)
3. **Query DOM state** — run `document.querySelector('.screen.active').id` after each navigation to confirm the correct screen is showing. Do NOT rely on screenshots or vision tools for verification.
4. **Animations work** — CSS transitions, hover effects, scroll animations play
5. **Navigation works** — internal links load, back button works, history stack is correct
6. **Content present** — all text, images, sections visible (not empty divs)
7. **Animations are unique** — motion language reflects the *specific app being cloned*, not a generic slide-fade pattern copied from another project

**Verification method:** Use `browser_console` to check DOM state and call JS functions. Click buttons via `browser_click` and confirm state changes. The user explicitly prefers click-through over screenshots — screenshots do not prove interactivity.

**Common failure: blank screen.** If the clone opens to a blank page, check the console for JS errors. On `file://` URLs, external `<script src="app.js">` may silently fail. Fix by inlining the JS into the HTML file. Verify with `typeof window.navigateTo` — if `undefined`, the script didn't execute.
